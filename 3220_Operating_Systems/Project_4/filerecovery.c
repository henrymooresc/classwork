...
/*	Exploration Functions==================================================================*/
	/* Function: createFile    */
		File *createFile(unsigned char *raw, char *filepath){
			...
			nextFile->pathLength = strlen(filepath);

			nextFile->filepath = (char *) malloc(nextFile->pathLength);
			for(int i = 0; i < nextFile->pathLength; i++){
	...
	/* Function: exploreDirectory */
		...
		void exploreDirectory(File *file){
			...
			for(int k = 0; k < j; k++)
			{
				dirname[k] = file->filename[k];
			}
			dirname[j] = '\0';

			char currpath[file->pathLength];
			for(int k = 0; k < file->pathLength; k++){
				currpath[k] = file->filepath[k];
			}

			//Get new filepath
			int tempsize = file->pathLength + j + 1;
			char *newfilepath = (char *) malloc(tempsize);
			memset(newfilepath, 0, (tempsize));
			strcpy(newfilepath, currpath);
			...
			while(z < i){
				...
				while((currentIndex + DIR_ENTR_SIZE) <= SECTOR_SIZE){
					if(data[sectorIndex+currentIndex] != FREE_CLUSTER){
						unsigned char *rawData = (unsigned char *) malloc(DIR_ENTR_SIZE);
